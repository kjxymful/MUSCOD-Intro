...
const int NRC  = 4;  /* Number of coupled constraints */   //4 ?? was sind coupled constraints?
...
Model *model = NULL;
std::string model_file="model_hoppingrobot.lua";
std::string result_file="RES/meshup_hoppingrobot.csv";
...
void update_generalized_variables (double *xd, double *u, 
  VectorNd &Q, VectorNd &QDot, VectorNd &Tau) 
{
...
static void mfcn(double *ts, double *sd, double *sa, double *p, double *pr,
  double *mval,  long *dpnd, InfoPtr *info) 
{
  ...
  update_generalized_variables (sd, NULL, Q, QDot, Tau);
  
  // Compute Contact Impulses with RBDL
  // hint: Define vector QDotPlus (all zeros) first;
  VectorNd QDotPlus = VectorNd::Zero(model->dof_count); 
	ComputeConstraintImpulsesDirect(*model,Q,QDot,constraint_set,QDotPlus);

  double impulse_x = constraint_set.impulse[1];

  *mval = impulse_x*impulse_x;  //warum impuls**2
...
/** \brief Constraints during the flight */
  ...
  double *p, double *pr, double *res, long *dpnd, InfoPtr *info)
{
  ...
  VectorNd contact_point_local = CalcBodyToBaseCoordinates(*model, Q, model->GetBodyId("LegEnd"),Vector3d::Zero(model->dof_count));
  
  res[0] = Q[1]-contact_point_local[0]; //0 oder contact point?
  
}

/** \brief Constraints at touchdown */
  ...
  double *p, double *pr, double *res, long *dpnd, InfoPtr *info)
{
  ...
  unsigned int LegEndID=model->GetBodyId("LegEnd");
  VectorNd LegEnd_origin = CalcBodyToBaseCoordinates (*model, Q, LegEndID, Vector3d (0., 0., 0.));
  VectorNd contact_point_local =CalcBodyToBaseCoordinates(*model, Q, model->GetBodyId("LegEnd"), Vector3d::Zero(model->dof_count));
  VectorNd contact_point_velocity =CalcPointVelocity(*model,Q,QDot,LegEndID,contact_point_local,true);
  
  res[0] = Q[1]-contact_point_local[2]; //wieso?
  res[1] = QDot[1]-contact_point_velocity[2];
}

/** \brief Constraints during contact */
  ...
  double *p, double *pr, double *res, long *dpnd, InfoPtr *info)
{
  ...
  res[0] = constraint_set.force[1]; // force greater zero  //wo sagt das, dass force greater 0
  ...
  double *p, double *pr, double *res, long *dpnd, InfoPtr *info)
{
  ...
  res[0] = constraint_set.force[1]; // force greater zero
  res[1] = constraint_set.v_plus[1]-5; // velocity greater five  // warum sagt das größer 5? bzw wie inequality?
  
}

/** \brief Periodicity constraints (coupling of start and end point) */
  ...
  double *p, double *pr, double *res, long *dpnd, InfoPtr *info)
{
  ...
  res[0] = -Q[0]-0;   // oder -Q bzw wo ist der Unterschied?
  res[1] = -Q[1]-0;
  res[2] = -QDot[0]-0;
  res[3] = -QDot[1]-0;
...
static void data_out( double *t, double *sd, double *sa, double *u, 
...
void def_model(void)
{
  ...
  def_mstage( 2,
			  ...
			  NULL, lfcn,
			  0, 0, 0, 
			  NULL, ffcn_contact, NULL,
			  NULL, NULL
			);

  /* Point Constraints Contact phase */
  def_mpc(0, "Start Point", NPR, 0, 0, NULL, rcfcn_periodic_s);
  def_mpc(0, "Interior Point", NPR, 1, 1, rdfcn_flight_i, NULL);
  
  def_mpc(1, "Start Point", NPR, 2, 2, rdfcn_touchdown_s, NULL);
