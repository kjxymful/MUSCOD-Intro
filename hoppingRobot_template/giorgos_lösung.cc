...
Vector3d contact_point_velocity;
  ...
  double *p, double *rhs, double *rwh, long *iwh, InfoPtr *info)
{
	...
	ForwardDynamics (*model, Q, QDot, Tau, QDDot, NULL);
...
const int NRD_flight = 1;
const int NRDE_flight = 0;
  ...
  double *p, double *pr, double *res, long *dpnd, InfoPtr *info)
{
  if (*dpnd) {
    *dpnd = RFCN_DPND(0, *sd, 0, 0, 0, 0);
	...
	contact_point_local = LegEnd_origin ;

	res[0] = Q[1] - contact_point_local[0];
}


const int NRD_touchdown = 2;
const int NRDE_toucdown = 2;
  ...
  double *p, double *pr, double *res, long *dpnd, InfoPtr *info)
{
  if (*dpnd) {
    *dpnd = RFCN_DPND(0, *sd, 0, 0, 0, 0);
	...
	contact_point_velocity = CalcPointVelocity(*model, Q, QDot, LegEnd_id, contact_point_local);

	res[0] = Q[1] - contact_point_local[2];
	res[1] = QDot[1] - contact_point_velocity[2];
}



const int NRD_contact_i = 1;
const int NRDE_contact_i = 0;
...
const int NRD_contact_e = 2;
const int NRDE_contact_e = 0;
  ...
  double *p, double *pr, double *res, long *dpnd, InfoPtr *info)
{
  if (*dpnd) {
    *dpnd = RFCN_DPND(0, *sd, 0, *u, 0, 0);
	...
	res[0] = constraint_set.force[1]; // force greater zero
	res[1] = constraint_set.v_plus[1]; // velocity greater five
}
...
void def_model(void)
{
	...
	def_mpc(0, "Interior Point", NPR, NRD_flight, NRDE_flight, rdfcn_flight_i, NULL);

	def_mpc(1, "Start Point", NPR, NRD_touchdown, NRDE_toucdown, rdfcn_touchdown_s, NULL);

	def_mpc(2, "Interior Point", NPR, NRD_contact_i, NRDE_contact_i, rdfcn_contact_i, NULL);
	def_mpc(2, "End Point", NPR, NRDE_contact_e, NRD_contact_e, rdfcn_contact_e, rcfcn_periodic_e);
